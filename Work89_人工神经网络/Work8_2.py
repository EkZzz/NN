...
# ---设置模型参数初值w
np.random.seed(612)
w1 = tf.Variable(np.random.randn(5, 16), dtype=tf.float32)
...
for i in range(0, iter + 1):
    with tf.GradientTape(persistent=True) as tape:
        hidden_train = tf.nn.relu(tf.matmul(X_train, w1))
        pred_train = tf.nn.softmax(tf.matmul(hidden_train, w2))
        loss_train = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(y_true=Y_train, y_pred=pred_train))
    
    hidden_test = tf.nn.relu(tf.matmul(X_test, w1))
    pred_test = tf.nn.softmax(tf.matmul(hidden_test, w2))
    loss_test = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(y_true=Y_test, y_pred=pred_test))

    grads = tape.gradient(loss_train, [w1,w2])
    w1.assign_sub(learn_rate*grads[0])
    w2.assign_sub(learn_rate*grads[1])
